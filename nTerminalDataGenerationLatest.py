...
CI_RATES = [0, .2]  # CI rates to test (percentage of vessels with CI capability)
...
def generate_terminal_subsets(num_terminals: int) -> List[List[int]]:
    ...
    where each part is at least 1.
    """
    if num_terminals < 3:
        return []

    subsets = []
    remaining_terminals = num_terminals - 3
    for p in range(remaining_terminals + 1):
        for b in range(remaining_terminals - p + 1):
            h = remaining_terminals - p - b
            subsets.append([p + 1, b + 1, h + 1])
    return subsets
...
def print_simulation_data_stats(vessel_data: Dict[str, np.ndarray], terminal_names: List[str]):
    ...
    print(f"  {'Terminal':<25} {'Vessels':<10} {'Total Volume (TEU)':<20} {'Avg Volume/Vessel':<18} {'CI Vessels':<12} {'CI %':<8}")
    ...
    for terminal_idx in range(num_terminals):
        ...
        terminal_name = terminal_names[terminal_idx] if terminal_idx < len(terminal_names) else f"Terminal {terminal_idx + 1}"

        print(f"  {terminal_name:<25} {terminal_vessels:<10.0f} {terminal_volume:<20,.0f} {avg_volume_per_vessel:<18,.0f} {ci_vessels:<12.0f} {ci_percentage:<8.1f}%")
    ...
    print(f"  {'-'*25} {'-'*10} {'-'*20} {'-'*18} {'-'*12} {'-'*8}")
    print(f"  {'TOTAL/AVERAGE':<25} {total_vessels:<10.0f} {total_volume:<20,.0f} {overall_avg_volume:<18,.0f} {total_ci_vessels:<12.0f} {overall_ci_percentage:<8.1f}%")
