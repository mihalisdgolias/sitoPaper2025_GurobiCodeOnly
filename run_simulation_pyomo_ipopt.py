...
It features two parallel solution paths to ensure robustness and comparison:
Gurobi (Exact PWL) and Pyomo (Sigmoid Approximation).
...
PIECEWISE COST FUNCTION: The model uses a two-phase cost function to reflect
terminal economics:
1. Phase 1 (u <= u_opt): Decreasing Marginal Cost (MC).
2. Phase 2 (u > u_opt): Increasing Marginal Cost (MC).
...
- Q[i]: Continuous. Total cargo volume handled by terminal i (TEU/week).
- u[i]: Continuous. Utilization rate of terminal i (Q[i] / C[i]).
- TC[i]: Continuous. Total Production Cost of terminal i (Piecewise/Sigmoid).
- P[i]: Continuous. Total Profit of terminal i after cooperation/transfers.
- F[i]: Continuous. Transfer Fee ($/TEU) set by terminal i.
- P_min: Continuous. Minimum profit across all terminals (MAXMIN objective only).

KEY PARAMETERS (Input Data/Calculated Baseline):
- V[j]: Volume of cargo for vessel j (TEU).
- C[i]: Capacity of terminal i (TEU/week).
- u_opt[i]: Optimal utilization point where MC is minimized.
- mc_start[i]: Initial Marginal Cost (MC at u=0).
- slope1[i]: Cost decrease rate (Phase 1).
- slope2[i]: Cost increase rate (Phase 2).
- S: CI capability subsidy ($/TEU).
- CI_term[i]: Binary (0/1). Terminal i's CI-capability status for the scenario.
- R_before[i]: Terminal i's fixed revenue in the initial state.
- TC_before[i]: Terminal i's total cost in the initial state.
- P_before[i]: Terminal i's profit in the initial state.

--------------------------------------------------------------------------------
3. CORE CONSTRAINTS AND HARDCODED DATA
--------------------------------------------------------------------------------

OBJECTIVES:
- MAXPROF: Maximize sum(P[i]).
- MAXMIN: Maximize P_min.

CONSTRAINTS:
1. Vessel Assignment: sum(x[j, i] over i) = 1.
2. Volume Conservation: Total volume before = Total volume after.
3. Cost Link (Gurobi): TC[i] linked to u[i] via **GenConstrPWL** (Exact PWL).
4. Cost Link (Pyomo): TC[i] linked to u[i] via **Sigmoid Smoothing** (Approximation).
5. Profit Calculation: P[i] = R_before[i] + P_transfer[i] - (TC[i] - TC_before[i]).
6. **Profit Stability (Participation Constraint)**: P[i] >= 0.99 * P_before[i] (Hardcoded 99% floor).

HARDCODED DATA POINTS:
- **Solver Time Limit**: SimulationConfig.TIME_LIMIT = 300 seconds.
- **Minimum Volume Constraint**: Q[i] >= 0.1 * C[i]. (Terminals must operate above 10% capacity).
- **Profit Stability Factor**: 0.99 (Terminals must retain at least 99% of pre-cooperation profit).
- **Pyomo Smoothing Steepness**: k = 10000 (Used in the Sigmoid function to create a very sharp, near-discontinuous transition for the Pyomo model).
- **Optimized Fee Bounds**: F[i] is bounded between $0.0 and $1000.0/TEU for 'optimized' pricing.
"""
...
import logging
import time
# CORRECTED: Added Tuple to the import list
...
def calculate_exact_cost(terminal_idx, utilization, terminal_capacities, cost_initial, cost_slope1, cost_slope2, optimal_utilization):
...
def calculate_marginal_cost(terminal_idx: int, utilization: float, optimal_utilization, cost_initial, cost_slope1, cost_slope2) -> float:
...
def calculate_marginal_profit(terminal_idx: int, utilization: float, revenue_decrease_rate, revenue_initial_charge, optimal_utilization, cost_initial, cost_slope1, cost_slope2) -> float:
    ...
    marginal_cost = calculate_marginal_cost(terminal_idx, utilization, optimal_utilization, cost_initial, cost_slope1, cost_slope2)
...
def validate_input_data(input_data_ves: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if len(input_data_ves['c']) != num_terminals:
            logger.error(f"Capacity array length {len(input_data_ves['c'])} doesn't match num_terminals {num_terminals}")
            return False
...
# FIXED: Function definition with correct Tuple type hint import
def calculate_baseline_state(input_data_ves: Dict[str, Any]) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
def vessel_based_opt_gurobi(input_data_ves: Dict[str, Any], pricing_mechanism: str = 'optimized') -> Optional[Dict[str, Any]]:
    ...
    volume_before, utilization_before, revenue_before, cost_before, profit_before = calculate_baseline_state(input_data_ves)
    ...
    for objective_name in ['MAXPROF', 'MAXMIN']:
        try:
            ...
            if pricing_mechanism == 'optimized':
                ...
                profit_factor_after = {}
                for i in range(num_terminals):
                    mp = calculate_marginal_profit(i, utilization_before[i], revenue_decrease_rate, revenue_initial_charge, optimal_utilization, cost_initial, cost_slope1, cost_slope2)
            ...
            for i in range(num_terminals):
                # Volume Calculation
                model.addConstr(volume_after[i] == gp.quicksum(vessel_volumes[j] * vessel_assignment[j, i] for j in range(num_vessels)), name=f"C_Q_{i}")
                ...
                if pricing_mechanism == 'marginal_cost':
                    ...
                    mc_vals = [max(0, calculate_marginal_cost(i, u, optimal_utilization, cost_initial, cost_slope1, cost_slope2)) for u in u_vals]
                    model.addGenConstrPWL(utilization_after[i], profit_factor_after[i], u_vals, mc_vals, name=f'C_F_MC_{i}')
                ...
                model.addGenConstrPWL(utilization_after[i], production_cost[i], u_points, y_points, name=f'C_TC_PWL_{i}')

                # Transfer Profit Calculation
                transfer_expr = gp.LinExpr(0)
                for j in range(num_vessels):
                    for k in range(num_terminals):
                        fee_source = profit_factor_after[k] if pricing_mechanism != 'marginal_profit' else profit_factor_after[k]
                        fee_dest = profit_factor_after[i] if pricing_mechanism != 'marginal_profit' else profit_factor_after[i]

                        # Vessel j initially at k, moves to i (i receives fee F_k)
                        if vessel_assignments[j, k] == 1 and i != k:
                            transfer_expr += vessel_assignment[j, i] * (fee_source * vessel_volumes[j] + ci_terminals[i] * subsidy * vessel_ci_capabilities[j, k] * vessel_volumes[j])

                        # Vessel j initially at i, moves to k (i pays fee F_i)
                        if vessel_assignments[j, i] == 1 and i != k:
                            # Note: Original code used a subtraction here: transfer_expr -= vessel_assignment[j, k] * (fee_dest * vessel_volumes[j] + ...)
                ...
                model.addConstr(profit_terminal[i] == revenue_before[i] + profit_from_transfers[i] - (production_cost[i] - cost_before[i]), name=f"C_P_total_{i}")
            ...
            # Global Constraints
            for j in range(num_vessels):
                model.addConstr(gp.quicksum(vessel_assignment[j, i] for i in range(num_terminals)) == 1, name=f"C_x_sum_{j}")

            # Volume Conservation
            model.addConstr(gp.quicksum(volume_after[i] for i in range(num_terminals)) == np.sum(volume_before), name="C_Q_cons")
...
def vessel_based_opt_pyomo_minlp(input_data_ves: Dict[str, Any], pricing_mechanism: str = 'optimized') -> Optional[Dict[str, Any]]:
    """
    ...
    for i in range(num_terminals):
        ...
        cost_at_optimal_list.append((input_data_ves['cost_initial'][i] * u_opt - 0.5 * input_data_ves['cost_decrease_rate'][i] * u_opt ** 2) * input_data_ves['c'][i])
    ...
    for objective_name in objective_names:
        try:
            ...
            model.u_opt = pyo.Param(model.terminals, initialize={i: input_data_ves['optimal_vcr_cost_point'][i] for i in range(num_terminals)})
            model.mc_start = pyo.Param(model.terminals, initialize={i: input_data_ves['cost_initial'][i] for i in range(num_terminals)})
            model.slope1 = pyo.Param(model.terminals, initialize={i: input_data_ves['cost_decrease_rate'][i] for i in range(num_terminals)})
            model.slope2 = pyo.Param(model.terminals, initialize={i: input_data_ves['cost_increase_rate'][i] for i in range(num_terminals)})
            ...
            model.A_before = pyo.Param(model.vessels, model.terminals, initialize={(j, i): input_data_ves['x'][j, i] for j in range(num_vessels) for i in range(num_terminals)})
            model.CI_term = pyo.Param(model.terminals, initialize={i: input_data_ves['ciTerm'][i] for i in range(num_terminals)})
            model.CI_ves = pyo.Param(model.vessels, model.terminals, initialize={(j, i): input_data_ves['xCI'][j, i] for j in range(num_vessels) for i in range(num_terminals)})
            model.S = pyo.Param(initialize=input_data_ves['subPar'])
            model.R_dec_rate = pyo.Param(model.terminals, initialize={i: -input_data_ves['decrease_rate'][i] for i in range(num_terminals)})
            model.R_init_charge = pyo.Param(model.terminals, initialize={i: input_data_ves['initial_charge'][i] for i in range(num_terminals)})
            ...
            if pricing_mechanism == 'marginal_profit':
                F_fixed = {}
                for i in range(num_terminals):
                    mp = calculate_marginal_profit(i, utilization_before[i], model.R_dec_rate[i], model.R_init_charge[i], model.u_opt, model.mc_start, model.slope1, model.slope2)
                    F_fixed[i] = max(0, mp)
            ...
            for i in model.terminals:
                ...
                TC2 = cost_at_opt + (mc_at_opt * (model.u[i] - u_opt) + 0.5 * model.slope2[i] * (model.u[i] - u_opt)**2) * model.C[i]

                # Sigmoid approximation (k=10000 for steep transition)
                k = 10000
                sigmoid = 1 / (1 + pyo.exp(-k * (model.u[i] - u_opt)))

                model.add_component(f'cost_link_{i}', pyo.Constraint(expr=model.TC[i] == TC1 * (1 - sigmoid) + TC2 * sigmoid))
                ...
                if pricing_mechanism == 'marginal_cost':
                    ...
                    model.add_component(f'marginal_cost_constraint_{i}', pyo.Constraint(expr=model.F[i] == pyo.Expr.max(0, model.mc_start[i] - model.slope1[i] * model.u[i])))
                ...
                for j in model.vessels:
                    for k in model.terminals:
                        ...
                        if input_data_ves['x'][j, k] == 1 and i != k:
                            P_transfer_expr += model.x[j, i] * (F_var_k * model.V[j] + model.CI_term[i] * model.S * model.CI_ves[j, k] * model.V[j])

                        # Vessel j initially at i, moves to k (i pays)
                        if input_data_ves['x'][j, i] == 1 and i != k:
                            P_transfer_expr -= model.x[j, k] * (F_var_i * model.V[j] + model.CI_term[i] * model.S * model.CI_ves[j, i] * model.V[j])

                model.add_component(f'profit_calc_{i}', pyo.Constraint(expr=model.P[i] == model.R_before[i] + P_transfer_expr - (model.TC[i] - model.TC_before[i])))

                # C7: Profit Stability
                def profit_stability_rule(model, i):
                    return model.P[i] >= 0.99 * model.P_before[i]
                model.add_component(f'profit_stability_{i}', pyo.Constraint(model.terminals, rule=profit_stability_rule))

            # C4: Volume Conservation (global)
            model.volume_conservation = pyo.Constraint(expr=sum(model.Q[i] for i in model.terminals) == np.sum(volume_before))
            ...
            if (result.solver.status == pyo.SolverStatus.ok and
                result.solver.termination_condition in [pyo.TerminationCondition.optimal, pyo.TerminationCondition.locallyOptimal, pyo.TerminationCondition.feasible]):
        ...
        for subsidy in subsidies:
            ...
            for ci_combo_idx in range(num_ci_combinations):
                for pricing_mechanism in pricing_mechanisms:
                    ...
                    ci_terminals_str = '_'.join([str(t + 1) for t in ci_terminals_list]) if ci_terminals_list.size > 0 else 'None'
...
def save_results_to_excel_with_retry(results: Dict[str, Any], filepath: str):
    ...
    solver_used = results['simulation_params']['solver_used']
