...
def vessel_based_opt(input_data_ves: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
    Terminal cooperation optimization using Gurobi with fixed revenue rates (corrected version).

    Key fixes:
    1. Revenue rate per container stays fixed (matches GAMS model)
    2. Removed artificial 0.8 cost reduction factor
    3. Revenue changes only from transfers and subsidies, not base rates
            ...
            try:
                ...
                for i in range(num_terminals):
                    ...
                    def calculate_total_cost_pwl(u_values):
                        ...
                        costs[mask1] = cost_initial[i] * u_values[mask1] - 0.5 * cost_slope1[i] * u_values[mask1] ** 2
                        ...
                        if np.any(mask2):
                            ...
                            costs[mask2] = (cost_at_optimal +
                                            (cost_initial[i] - cost_slope1[i] * optimal_utilization[i]) *
                                            (u_values[mask2] - optimal_utilization[i]) +
                                            0.5 * cost_slope2[i] * (u_values[mask2] - optimal_utilization[i]) ** 2)

                        return costs * terminal_capacities[i]
                    ...
                    # Piecewise linear cost function - FIXED: No artificial cost reduction
                    u_vals = np.linspace(0, 0.99, 15)  # Increased points for better approximation
                    x_points = np.insert(u_vals, np.searchsorted(u_vals, optimal_utilization[i]),
                                         optimal_utilization[i])
                    y_points = calculate_total_cost_pwl(x_points)
                    # we need aritifical reduction of 1% for feasibility
                    y_points_actual = (.99*y_points)

                    model.addGenConstrPWL(utilization_after[i], production_cost[i], x_points, y_points_actual,
