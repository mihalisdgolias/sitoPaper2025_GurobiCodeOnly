...
class SimulationConfig:
    ...
    # --- Simulation Parameters from nTerminalDataGenerationLatest.py ---
    # change this to match the nTerminalDataGenerationLatest.py
    TERMINALS_TO_SIMULATE = [3]
    NUM_VESSEL_INSTANCES = 1
    SUBSIDIES_TO_SIMULATE = [0]

    # --- Vessel and Terminal Parameters (from nTerminalDataGenerationLatest.py) ---
   # VESSEL_VOLUME_MIN = 850
   # VESSEL_VOLUME_MAX = 5000
   # TERMINAL_CAPACITY_MIN = 25000000 / (52 * 4)  # Adjust for n=4 terminals
   # TERMINAL_CAPACITY_MAX = 35000000 / (52 * 4)  # Adjust for n=4 terminals
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('simulation_linear_cost.log'),
...
def calculate_linear_cost_approximation(cost_initial: float, cost_slope1: float, cost_slope2: float,
                                        optimal_utilization: float, capacity: float,
                                        utilization_range: tuple = (0.1, 0.9)) -> tuple:
    """
    Calculate linear approximation coefficients for the total cost function.
    ...
    We approximate this with: TC_linear = alpha + beta * utilization * capacity

    Args:
        cost_initial: Initial marginal cost parameter
        cost_slope1: Cost decrease rate in phase 1
        cost_slope2: Cost increase rate in phase 2
        optimal_utilization: Transition point between phases
        capacity: Terminal capacity
        utilization_range: Range of utilization to fit over (min_u, max_u)

    Returns:
        (alpha, beta): Linear approximation coefficients where TC = alpha + beta * utilization * capacity
    """

    min_u, max_u = utilization_range

    # Calculate fixed mc_min based on continuity constraint
    mc_min = cost_initial - cost_slope1 * optimal_utilization

    # Create sample points across utilization range
    u_points = np.linspace(min_u, max_u, 20)
    tc_points = np.zeros_like(u_points)

    # Calculate actual total cost at each point
    for i, u in enumerate(u_points):
        if u <= optimal_utilization:
            # Phase 1: decreasing marginal cost
            tc_points[i] = capacity * (cost_initial * u - 0.5 * cost_slope1 * u ** 2)
        else:
            # Phase 2: increasing marginal cost
            tc_at_optimal = capacity * (
                        cost_initial * optimal_utilization - 0.5 * cost_slope1 * optimal_utilization ** 2)
            additional_cost = capacity * (mc_min * (u - optimal_utilization) +
                                          0.5 * cost_slope2 * (u - optimal_utilization) ** 2)
            tc_points[i] = tc_at_optimal + additional_cost

    # Fit linear approximation: TC = alpha + beta * (utilization * capacity)
    # Rearranging: TC = alpha + beta * utilization * capacity
    # We'll fit: TC = alpha + beta_raw * utilization, then beta = beta_raw * capacity

    # Perform least squares fit
    A = np.vstack([np.ones(len(u_points)), u_points]).T
    coefficients = np.linalg.lstsq(A, tc_points, rcond=None)[0]
    alpha = coefficients[0]
    beta_raw = coefficients[1]

    # Since we want TC = alpha + beta * utilization * capacity,
    # and we fitted TC = alpha + beta_raw * utilization,
    # we have beta_raw = beta * capacity, so beta = beta_raw / capacity
    beta = beta_raw / capacity
...
def vessel_based_opt_linear_cost(input_data_ves: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
    Terminal cooperation optimization using Gurobi with LINEAR COST APPROXIMATION.

    Key change from original:
    - Replaced piecewise linear cost approximation with simple linear approximation
    - Cost function: TC = alpha + beta * utilization * capacity
    ...
    try:
        ...
        # Calculate base revenue rate that stays constant (like GAMS pfBefore)
        ...
        # NEW: Calculate linear cost approximation coefficients for each terminal
        linear_cost_params = []
        logger.info("Calculating linear cost approximations:")
        for i in range(num_terminals):
            alpha, beta = calculate_linear_cost_approximation(
                cost_initial[i], cost_slope1[i], cost_slope2[i],
                optimal_utilization[i], terminal_capacities[i]
            )
            linear_cost_params.append((alpha, beta))
            logger.debug(f"Terminal {i + 1}: TC_linear = {alpha:.0f} + {beta:.2f} * utilization * capacity")

        # Calculate cost before cooperation using SAME linear approximation
        cost_before = np.zeros(num_terminals)
        for i in range(num_terminals):
            alpha, beta = linear_cost_params[i]
            utilization = utilization_before[i]
            cost_before[i] = alpha + beta * utilization * terminal_capacities[i]
        ...
        results = {
            ...
            'linear_cost_params': linear_cost_params  # Store for debugging
        }

        # Solve for both objectives
        for objective_name in ['MAXPROF', 'MAXMIN']:
            try:
                model = gp.Model(f"Terminal_Cooperation_LinearCost_{objective_name}")
                ...
                for i in range(num_terminals):
                    ...
                    # NEW: LINEAR COST APPROXIMATION
                    # Replace complex piecewise cost with simple linear: TC = alpha + beta * utilization * capacity
                    alpha, beta = linear_cost_params[i]
                    model.addConstr(
                        production_cost[i] == alpha + beta * utilization_after[i] * terminal_capacities[i],
                        name=f"linear_cost_constraint_{i}"
                    )

                    # Revenue calculation with FIXED rate per container (like GAMS)
                ...
                if model.status == GRB.OPTIMAL:
                    ...
                    logger.info(f"Linear cost optimization {objective_name} completed successfully")

                elif model.status == GRB.INFEASIBLE:
                    logger.warning(f"Linear cost optimization {objective_name} infeasible - no cooperation possible")
                    ...
                    logger.warning(f"Linear cost optimization {objective_name} failed with status: {model.status}")
                ...
                logger.error(f"Linear cost optimization {objective_name} failed: {e}")
                results[f'profitAfter_{objective_name}'] = profit_before.copy()
                results[f'volumeAfter_{objective_name}'] = volume_before.copy()
                results[f'objval_{objective_name}'] = np.nan
                results[f'optimalityGap_{objective_name}'] = np.nan
        ...
        if maxprof_infeasible or maxmin_infeasible:
            ...
            results['feasibility_status'] = 'Feasible and Sensible - Linear Cost Approximation'
        ...
        logger.error(f"Critical linear cost optimization error: {e}")
...
def save_results_to_excel_with_retry(results: Dict[str, Any], filepath: str):
    ...
    for obj_name in ['MAXPROF', 'MAXMIN']:
        ...
        for i in range(num_terminals):
            df_data.append({
                ...
                'Cost_Method': 'Linear_Approximation'
            })

    df = pd.DataFrame(df_data)

    # Use CSV instead of Excel to avoid corruption issues
    csv_filepath = filepath.replace('.xlsx', '_linear_cost.csv')
    ...
    try:
        ...
        logger.info(f"Linear cost results saved to CSV successfully")
    except Exception as e:
...
def run_all_combinations():
    """
    Main function to orchestrate the entire simulation with LINEAR COST APPROXIMATION.
    """
    ...
    results_folder = "simulation_results_linear_cost"
    excel_results_file = os.path.join(results_folder, "summary_results_linear_cost.xlsx")
    ...
    subsidies = np.array(SimulationConfig.SUBSIDIES_TO_SIMULATE)
    ...
    logger.info("=== STARTING LINEAR COST APPROXIMATION SIMULATION ===")
    logger.info(f"Cost method: Linear approximation (TC = alpha + beta * utilization * capacity)")
    ...
    logger.info(f"Estimated runtime: {total_combinations * 0.05 / 60:.1f} minutes (faster due to linear cost)")
    ...
    for filename in tqdm(data_files, desc="Processing data files"):
        ...
        logger.info(
            f"\n--- Processing data for {num_terminals} terminals, CI Rate: {ci_rate:.0%}, Instance: {instance_idx} (LINEAR COST) ---")
        ...
        for subsidy in tqdm(subsidies, desc="Processing subsidies", leave=False):
            ...
            for ci_combo_idx in tqdm(range(num_ci_combinations), desc="Processing CI combinations", leave=False):
                ...
                # Run LINEAR COST optimization
                result = vessel_based_opt_linear_cost(input_data_ves)

                if result is None:
                    logger.error(f"Linear cost optimization failed critically for combination {combination_count}")
                ...
                output_filename = f"{filename_base}_Sub{int(subsidy * 100):02d}_CICombo_{ci_terminals_str}_LinearCost.pkl"
    ...
    # Final summary
    logger.info("\n=== LINEAR COST APPROXIMATION SIMULATION COMPLETE ===")
